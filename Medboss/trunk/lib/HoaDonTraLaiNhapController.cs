namespace Nammedia.Medboss.lib
{
    ...
    internal class HoaDonTraLaiNhapController : DataController, IFinder
    {
        public bool ThuHayChiSearchParam;

        public ArrayList ConditionalList(string Condition)
        {
            string text = "select * from HoaDonNhapTraLai";
            if (Condition != "")
            {
            ...
            {
                text = "select MaHoaDon,MaThuocTraoDoi,SoLuong,DonGia,HanDung,SoLo,GhiChu,NgayNhap,STT from HoaDonNhapTraLaiChiTiet where MaHoaDon=" + info2.MaHoaDon + " order by STT";
                base.db.Command.CommandText = text;
        ...
        public int Delete(int MaHoaDon)
        {
            ...
            try
            {
                string text = "delete * from HoaDonNhapTraLaiChiTiet where MaHoaDon=" + MaHoaDon;
                command.CommandText = text;
                if (command.ExecuteNonQuery() <= 0)
                {
                    throw new DeleteException();
                }
                text = "delete * from HoaDonNhapTraLai where MaHoaDon=" + MaHoaDon;
                command.CommandText = text;
                if (command.ExecuteNonQuery() <= 0)
                {
                    throw new DeleteException();
        ...
        public int Insert(HoaDonTraLaiInfo ordTraLai)
        {
            ...
            try
            {
                missingId = IdManager.GetMissingId("MaHoaDon", "HoaDonNhapTraLai");
                ordTraLai.MaHoaDon = missingId;
                string text = string.Concat(new object[] { 
                    "insert into HoaDonNhapTraLai values(", ordTraLai.MaHoaDon, ",", ordTraLai.MaQuay, ",#", ordTraLai.Ngay.ToString("MM/dd/yyyy"), "#,", ordTraLai.HoaDonNhapBan, ",'", ordTraLai.GhiChu, "',", ordTraLai.MaNhanVien, ",", ordTraLai.TienTraLai, ",", ordTraLai.ThuHayChi, 
                ...
                foreach (TraLaiChiTietInfo info in ordTraLai.TraLaiChiTiet)
                {
                    ...
                    text = string.Concat(new object[] { 
                        "insert into HoaDonNhapTraLaiChiTiet values(", ordTraLai.MaHoaDon, ",", info.MaThuocTraoDoi, ",", info.SoLuong, ",", info.DonGia, ",", text2, ",'", info.SoLo, "','", info.GhiChu, "',#", info.NgayNhap.ToString("MM/dd/yyyy"), 
                        "#,", info.STT, ")"
                     });
                    command.CommandText = text;
                    if (command.ExecuteNonQuery() <= 0)
        ...
        private string para2str(FindParam[] findParams)
        {
            ...
            obj2 = text;
            return (string.Concat(new object[] { obj2, " Ngay between #", param.value, "# and #", param3.value, "#" }) + " and ThuHayChi=false").ToString();
        }
        ...
        public int Update(HoaDonTraLaiInfo traLaiInfo)
        {
            ...
            try
            {
                string text = string.Concat(new object[] { "update HoaDonNhapTraLai set MaQuay=", traLaiInfo.MaQuay, ",MaKhachHang=", traLaiInfo.MaKhachHang, ", Ngay=#", traLaiInfo.Ngay.ToString("MM/dd/yyyy"), "#, MaNhanVien=", traLaiInfo.MaNhanVien, " where MaHoaDon=", traLaiInfo.MaHoaDon });
                command.CommandText = text;
                if (command.ExecuteNonQuery() <= 0)
                {
                    throw new UpdateException();
                }
                text = "delete * from HoaDonNhapTraLaiChiTiet where MaHoaDon=" + traLaiInfo.MaHoaDon;
                command.CommandText = text;
                if (command.ExecuteNonQuery() <= 0)
                {
                    throw new DeleteException();
                ...
                foreach (TraLaiChiTietInfo info in traLaiInfo.TraLaiChiTiet)
                {
                    ...
                    text = string.Concat(new object[] { 
                        "insert into HoaDonNhapTraLaiChiTiet values(", traLaiInfo.MaHoaDon, ",", info.MaThuocTraoDoi, ",", info.SoLuong, ",", info.DonGia, ",", text2, ",'", info.SoLo, "','", info.GhiChu, "',#", info.NgayNhap.ToString("MM/dd/yyyy"), 
                        "#,", info.STT, ")"
                     });
                    command.CommandText = text;
                    if (command.ExecuteNonQuery() <= 0)
